...
- Added Forge build coverage for Minecraft 1.20.2, 1.20.4, 1.20.6, 1.21.1, 1.21.3, 1.21.4, 1.21.5, 1.21.6, 1.21.8, and 1.21.10.
...
- Unified every loader module on version 1.2.0 and added build/test coverage for all 24 declared targets.
- Added an event-driven installed-JAR acceptance harness; every declared target passed premium login, offline fallback, confirmed migration, and known-name denial on a fresh world.
...
- 新增 Minecraft 1.20.2、1.20.4、1.20.6、1.21.1、1.21.3、1.21.4、1.21.5、1.21.6、1.21.8 和 1.21.10 的 Forge 构建支持。
...
- 所有加载器模块统一使用 1.2.0 版本，并为全部 24 个已声明目标加入构建和测试覆盖。
- 新增事件驱动的生产 JAR 验收工具；所有已声明目标都在全新世界中通过正版登录、离线兜底、确认迁移和已验证名称拒绝测试。
