...
| Minecraft 1.21.1 | Forge 52.1.14 | 21 | Planned | Implemented independently in `platform/forge-1.21.1`; its JDK 21 build, shared fixtures, and codec/lifecycle tests pass, but no modded client/server matrix has run. |
...
scenarios listed in the handoff remain required before a release claim.
...
| 2026-07-12 | Forge 1.21.1 | Forge 52.1.14 / OpenJDK 21.0.11 | `platform/forge-1.21.1/build/libs/trueuuid-1.1.0-forge1.21.1.jar` | `:shared:protocol:test :platform:forge-1.21.1:build` passed. The adapter codec and lifecycle tests passed; safe endpoint verification is wired, but the real login acceptance matrix is pending. |
...
Old release lines are retained as read-only `archive/*` branches. The local
`1.20` branch currently duplicates `archive/forge-1.20-pre-monorepo`; do not
delete or rewrite it until a maintainer has confirmed remote archival and
release references. The historical NeoForge line is not a Forge variant.
