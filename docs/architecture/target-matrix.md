...
| 2026-07-18 | Forge 1.20.1 | Forge 47.4.10 / Java 17.0.12 | repaired `platform/forge-1.20.1/build/libs/trueuuid-1.1.0-forge1.20.1.jar` | Installed directly in a non-Gradle dedicated server's `mods/` directory. The offline-mode production server loaded `trueuuid.mixins.json`, logged `TrueUUID 已经加载`, reached `Done (15.573s)!`, and reported zero refmap or Mixin apply errors. No premium client was used; premium re-validation remains a separate desktop run. |
