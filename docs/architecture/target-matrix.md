...
| Minecraft 1.21.10 | Forge 60.1.11 | 21 | Planned | `platform/forge-1.21.10`; recompiles `platform/forge-common` plus the `modern-matrix` (EventBus 7) seam, with module-local **record-era** copies of `ForgeAdapterRuntime`, `ForgeServerLoginMixin`, and `ForgeClientHandshakeMixin` — Minecraft 1.21.9+ made authlib's `GameProfile` a record, moved the client session service to `Minecraft.services()`, and removed `ServerPlayer.getServer()`. `forge-common` stays byte-identical (excluded via the module's `compileJava.source`, mirroring the NeoForge 1.21.10 mechanism). Build, mixin refmap, focused tests, the 24-target root build, and the structural release-JAR verifier pass (2026-07-18). No login run yet. Declares single-patch `[1.21.10,1.21.11)` (protocol 773 pairs with beta-only 1.21.9). `release: false`. |
| Minecraft 1.21.11 | Forge 61.1.9 | 21 | Planned | `platform/forge-1.21.11`, a **standalone build island**. Minecraft 1.21.11 / Forge 61 require ForgeGradle `[7.0.3,8)` on **Gradle 9.5**, incompatible with the repo's multi-loader Gradle 8.14 build (FG 6.0.54 fails on Forge 61's userdev with `duplicate entry: mcp/client/Start.class`). So this target is its own Gradle build with its own wrapper (`cd platform/forge-1.21.11 && ./gradlew build`), source-sharing `platform/forge-common` + `shared/protocol` via `srcDir` exactly like the in-tree modules — `forge-common` stays byte-identical. It carries the same record-era seams as 1.21.10 **plus** the 1.21.11 `ResourceLocation`→`net.minecraft.resources.Identifier` rename seams (`ForgeNetIds`, `ForgeAuthPayload`, `ForgeClientQueryDecodeMixin`, `TrueuuidClientOverlay`). No Mixin refmap is generated: 1.21+ official mappings need none (the SpongePowered AP and the Gradle-9-incompatible MixinGradle are both omitted; Mixin resolves targets by official name at load). Build passes and produces `trueuuid-1.2.0-forge1.21.11.jar` with the mixin config + `MixinConfigs` manifest (2026-07-18). It is **not** in the root `settings.gradle`, root build, `release/targets.json`, or the manifest-driven CI matrix yet — that release-pipeline integration is a tracked follow-up. Protocol 774, final legacy-scheme release: single-patch permanently. No login run. `release: false`. |
...
1.21.6 / 1.21.8 / 1.21.10 (seven in-tree targets sharing `platform/forge-common`;
1.21.10 adds module-local record-era seams) plus **1.21.11** as a standalone
ForgeGradle-7 / Gradle-9.5 build island. Forge now covers the full 1.20.1–1.21.11
range except 1.20.5 and 1.21.2, for which upstream Forge ships no loader. **NeoForge
...
| Forge 1.21.10 | Same core plus `modern-matrix` and module-local 1.21.9+ record-era seams | passed; structural release-JAR verifier passed (2026-07-18) | none | Per-target login matrix and the shared 1.21 feature backlog |
...
| NeoForge 1.21.11 | Recompile the 1.21.1 core plus GUI + record-era + Identifier-rename seams | passed (2026-07-18) | One Mojang premium login: verified UUID, signed skin, chat/HUD, and clean disconnect passed | Remaining login matrix and the shared 1.21 feature backlog |
...
| 2026-07-18 | NeoForge 1.21.11 | NeoForge 21.11.44 / Java 21 | Prism client JAR `trueuuid-1.2.0-neoforge1.21.11.jar` (SHA-256 `17d952edf74ee7b6df3af553e2c69acca4dd72e6652c9c1fc435065b11564c7b`) + Gradle `runServer` | Matching premium client and offline-mode development server: both loaded TrueUUID 1.2.0; the server logged `session-verified premium login` with the verified account UUID; the client received localized premium chat and showed the premium HUD; the signed skin rendered; disconnect was clean; no Mixin, refmap, profile, authentication, or texture errors remained. This proves the Mojang-success scenario only, not release approval. |
...
| Date | Target | Loader/JDK | Artifact | Result |
|---|---|---|---|---|
| 2026-07-18 | Forge 1.21.10 | Forge 60.1.11 / OpenJDK 21 | `platform/forge-1.21.10/build/libs/trueuuid-1.2.0-forge1.21.10.jar` | New target closing the Forge coverage gap to 1.21.10. Recompiles `forge-common` + `modern-matrix` with module-local 1.21.9+ record-era copies of `ForgeAdapterRuntime` / `ForgeServerLoginMixin` / `ForgeClientHandshakeMixin` (`GameProfile` record accessors, `Minecraft.services().sessionService()`, `level().getServer()`, `PropertyMap` construction); `forge-common` verified byte-identical. Build, mixin refmap, focused tests, the 24-target root build (239 actionable tasks), and the structural release-JAR verifier all pass. No login run. |
| 2026-07-18 | Forge 1.21.11 | Forge 61.1.9 / ForgeGradle 7.0.3 / Gradle 9.5.0 / OpenJDK 21 | `platform/forge-1.21.11/build/libs/trueuuid-1.2.0-forge1.21.11.jar` | Closes Forge coverage to 1.21.11 as a **standalone build island** (own wrapper), after confirming FG 6.0.54/Gradle 8.14 cannot (`duplicate entry: mcp/client/Start.class`). Forge's official 1.21.11 MDK pins Gradle 9.5.0 + ForgeGradle `[7.0.3,8)`; a spike built that MDK clean. The island source-shares `forge-common` + `shared/protocol` via `srcDir` (`forge-common` byte-identical), carries the record-era seams plus the 1.21.11 `ResourceLocation`→`Identifier` rename seams (`ForgeNetIds`/`ForgeAuthPayload`/`ForgeClientQueryDecodeMixin`/`TrueuuidClientOverlay`, diagnostic-verified), and omits the Mixin AP + MixinGradle entirely — 1.21+ official mappings need no refmap (`highlysuspect.agency`/MixinGradle docs), and MixinGradle 0.7 is Gradle-9-incompatible. `./gradlew build` in the island produces the jar with `trueuuid.mixins.json` + `MixinConfigs` manifest and no refmap. The root build (24 targets) and `validate-targets.sh` stay green (islands are excluded from the manifest check). Release-pipeline integration (manifest entry, CI job, structural verifier) is a tracked follow-up. No login run. |
...
incremental `build/libs` output. That explains the otherwise contradictory
runtime result, but the original file is unavailable, so this remains an
inference and the login evidence must be re-recorded against the repaired JAR.
