...
当前清单中的 24 个精确目标均已通过四项生产 JAR 真实运行验收：正版登录、
离线兜底、确认迁移和已验证名称拒绝。清单包括 Forge 1.20.1-1.21.10 的
11 个精确目标、Fabric 1.20.1，以及 NeoForge 1.20.1-1.21.11 的 12 个精确
目标。未列出的中间版本不应视为已支持；Fabric 1.20.2-1.21.11 尚未实现，
Forge 1.21.11 仍是未接入发布清单的独立构建。
...
在 TrueUUID 握手完成后，于客户端左上角显示小型账号状态标识。绿色表示正版验证通过，红色表示服务器已允许离线兜底。该标识只在连接 TrueUUID 服务器后显示。
