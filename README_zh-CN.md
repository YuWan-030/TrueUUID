...
它也支持已配置的 Yggdrasil/authlib-injector 皮肤站账号。Forge 1.20.1 已通过真实运行验证；Forge 和 NeoForge 1.21.1 适配器正在完善中，并计划支持更多加载器和 Minecraft 版本。
...
`platform/<loader>-<minecraft-version>/build/libs/` 目录。Forge 1.20.1 是已通过真实运行验证的发布目标；准确的目标状态和发布规则见
[`docs/architecture/target-matrix.md`](docs/architecture/target-matrix.md)。
