...
仓库已按多个独立的加载器/游戏版本适配器组织，但目前没有其他已启用的构建目标。原有的
NeoForge 1.21.1 实现在历史分支中保留作参考；它不是当前分支中受支持的模块。
...
当前 Forge 1.20.1 构建产物会输出到
`platform/forge-1.20.1/build/libs/`。目标状态和发布规则见
