 ...
 *
 * <p>Loader-neutral authentication, presentation and verified-name persistence
 * live in shared modules. This class retains only NeoForge lifecycle and player
 * boundaries.
 */
public final class AdapterRuntime {
    ...
    public static synchronized void recordGraceLogin(GameProfile profile) {
        UUID id = NeoGameProfiles.id(profile);
    ...
    public static void onPlayerLoggedIn(PlayerEvent.PlayerLoggedInEvent event) {
        ...
        MinecraftServer server = NeoGameProfiles.server(player);
        ...
        Trueuuid.LOGGER.info("TrueUUID login_complete outcome={} player={} uuid={} auth_source={}",
                source.outcome(), NeoGameProfiles.name(player.getGameProfile()), player.getUUID(), source.authenticationSource());
        Trueuuid.acceptance("result={} player={} uuid={}",
                source == AuthenticationPresentation.OFFLINE_FALLBACK ? "offline_fallback" : "premium_join",
                NeoGameProfiles.name(player.getGameProfile()), player.getUUID());
        ...
        for (var delivery : LoginNotificationRouter.route(player, onlinePlayers,
                AdapterRuntime::hasOperatorPermission,
            ...
            if (delivery.kind() == LoginNotificationRouter.Kind.JOIN_RESULT) {
                ...
                delivery.recipient().sendSystemMessage(Component.translatable("trueuuid.operator.login",
                        NeoGameProfiles.name(player.getGameProfile()), player.getUUID(), source.outcome(),
                        source.authenticationSource()).withStyle(ChatFormatting.GRAY));
            }
        }
    ...
    public static void onPlayerLoggedOut(PlayerEvent.PlayerLoggedOutEvent event) {
        if (event.getEntity() instanceof ServerPlayer player) {
            ...
            activateGraceAfterLogout(NeoGameProfiles.name(player.getGameProfile()), player.getIpAddress());
        }
    }
