...
public final class TrueuuidConfig {
    ...
    private static final ForgeConfigSpec.BooleanValue RECENT_IP_GRACE_ENABLED;
    private static final ForgeConfigSpec.IntValue RECENT_IP_GRACE_TTL_SECONDS;
    ...
    static {
        ...
        OVERLAY_CORNER = builder.comment(
                ...
                // Forge 48 validates an absent value as null while creating a
    ...
    private static void setBoolean(CommentedFileConfig config, String key, ForgeConfigSpec.BooleanValue value) {
        Object current = config.get(key);
        if (current instanceof Boolean typed) value.set(typed);
    }

    private static void setLong(CommentedFileConfig config, String key, ForgeConfigSpec.LongValue value) {
        Object current = config.get(key);
        if (current instanceof Number typed) value.set(typed.longValue());
    }

    private static void setInt(CommentedFileConfig config, String key, ForgeConfigSpec.IntValue value) {
        Object current = config.get(key);
        if (current instanceof Number typed) value.set(typed.intValue());
    }

    private static void setDouble(CommentedFileConfig config, String key, ForgeConfigSpec.DoubleValue value) {
        Object current = config.get(key);
